...
1. 用户点击"打开标记文件"菜单项
2. 组件调用文件系统服务的 `dialogReadFile` 方法
3. 文件系统服务通过 IPC 调用主进程的 `dialog:open-file` 通道打开文件选择对话框
4. 用户选择文件后，主进程读取文件内容并返回
5. 业务逻辑层反序列化 JSON 数据为标记文件对象
6. 业务逻辑层更新 Redux Store
7. UI 更新显示图片和标注
...
4. 业务逻辑层调用文件系统服务的 `dialogWriteFile` 方法
5. 文件系统服务通过 IPC 调用主进程的 `dialog:save-file` 通道打开保存文件对话框
6. 用户选择路径后，主进程写入文件并返回结果
7. 显示保存成功提示

##### 6.3.4.3 添加图片流程

1. 用户点击"添加图片"菜单项
2. 组件调用文件系统服务的 `dialogReadImage` 方法
3. 文件系统服务通过 IPC 调用主进程的 `dialog:open-image` 通道打开图片选择对话框
4. 用户选择图片后，主进程加载图片并提取元数据
5. 创建图片信息对象
6. 业务逻辑层更新 Redux Store
7. UI 更新显示新标签页
...
1. 用户点击"导出为图片"菜单项
2. 组件调用文件系统服务的 `dialogWriteImage` 方法
3. 文件系统服务通过 IPC 调用主进程的 `dialog:save-image` 通道打开保存图片对话框
4. 用户选择路径后，主进程合并图片和标注为 PNG 并保存
5. 显示导出成功提示
