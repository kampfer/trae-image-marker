...
| -------- | --------------------------- | ---------------- | ---------------------------------------------------------- | ---------------------------------------------------------- |
| 新建     | `createNewFile()`           | 点击「新建」     | 始终可用；当前文件存在未保存修改时，执行前需弹出确认对话框 | [`createNewFile()`](./04-业务逻辑层.md#231-同步-actions)   |
| 打开     | `openFile()`                | 点击「打开」     | 始终可用                                                   | [`openFile()`](./04-业务逻辑层.md#232-异步-thunks)         |
| 保存     | `saveFile()`                | 点击「保存」     | 已打开文件且存在未保存修改，或当前为 Untitled 文件时可用   | [`saveFile()`](./04-业务逻辑层.md#232-异步-thunks)         |
| 另存为   | `saveFileAs()`              | 点击「另存为」   | 已打开文件时可用                                           | [`saveFileAs()`](./04-业务逻辑层.md#232-异步-thunks)       |
| 添加图片 | `addImage()`                | 点击「添加图片」 | 已打开标注文件时可用                                       | [`addImage()`](./04-业务逻辑层.md#33-actions)              |
...
sequenceDiagram
    ...
    FileDropdown->>Redux: dispatch addImage()
...
MainContent 组件本身不持有业务状态，仅通过 Redux 读取文件打开状态并做条件渲染，是连接布局框架与业务内容的路由性组件。Untitled 文件虽然尚未保存到磁盘，但在 Redux 中视为已打开文件，因此会正常进入文件工作区。
...
| ----------------- | -------------------- | -------------------------------------------------------- |
| `createNewFile()` | 点击「新建标注文件」 | [`createNewFile()`](./04-业务逻辑层.md#231-同步-actions) |
...
sequenceDiagram
    ...
    User->>Placeholder: 点击「新建标注文件」
    Placeholder->>Redux: dispatch createNewFile()
    Redux-->>Redux: 创建内存中的 Untitled 文件（filePath = null）
    Redux-->>Redux: 清理图片、标注、历史、画布和命令状态
    Redux-->>Redux: file.isOpened = true
...
TabsBar 聚合 antd Tabs 组件，为标注文件中的每张图片渲染一个标签页。标签页支持动态添加、切换和删除：添加标签页等同于添加新图片，用户点击添加按钮后调用 `addImage()` action；标签页数量过多时复用 antd Tabs 的溢出滚动交互。

每个标签页使用图片文件名作为标题，标题区域限制最大宽度为 150px。标题超出宽度时通过省略号截断，完整文件名通过无障碍文本或悬浮提示保留。标题宽度限制通过组件 CSS 类实现，不使用行内样式。

#### 组件 Props

| 属性名          | 类型             | 必填 | 默认值 | 来源                                                            | 说明                                                                 |
| --------------- | ---------------- | ---- | ------ | --------------------------------------------------------------- | -------------------------------------------------------------------- |
| `images`        | `ImageInfo[]`    | 是   | -      | Redux：[`selectImages`](./04-业务逻辑层.md#34-selectors)        | 当前标注文件中的图片列表，每个元素对应一个动态标签页；包含图片文件名 |
...
└── antd Tabs (type="editable-card")
    ├── Image Tab (title=图片文件名，max-width=150px)
    │   └── WorkSpace (每个 Tab 对应一个)
    └── Add Tab Button (点击后 dispatch addImage())
```

`TabsBar` 使用 antd Tabs 的 `items` 配置生成标签页，使用 `type="editable-card"` 提供添加和删除交互。标签页溢出时沿用 antd Tabs 的默认滚动和更多标签菜单行为，不额外实现滚动逻辑。

#### 空状态设计

当 `images` 数组为空时，TabsBar 仍渲染 antd Tabs 的添加按钮，用户可以通过该按钮调用 `addImage()` 添加第一张图片；添加成功后 Redux 更新 `images` 和 `activeImageId`，新标签页自动出现并激活。

#### 组件 Action

| Action 名                 | 调用时机             | 业务逻辑层定义                                |
| ------------------------- | -------------------- | --------------------------------------------- |
| `addImage()`              | 用户点击添加标签按钮 | [图片管理模块](./04-业务逻辑层.md#33-actions) |
| `setActiveImage(imageId)` | 用户点击标签页       | [图片管理模块](./04-业务逻辑层.md#33-actions) |
| `removeImage(imageId)`    | 用户点击标签关闭按钮 | [图片管理模块](./04-业务逻辑层.md#33-actions) |
...
sequenceDiagram
    ...
    User->>TabsBar: 点击添加标签按钮
    TabsBar->>Redux: dispatch addImage()
    Redux-->>Redux: image.images 新增图片，activeImageId 更新
    Redux-->>TabsBar: 状态变更通知
    TabsBar-->>User: 新标签页以图片文件名显示并自动激活
...
| 属性名      | 类型                                        | 必填 | 默认值 | 来源                                                        | 说明                                                           |
| ----------- | ------------------------------------------- | ---- | ------ | ----------------------------------------------------------- | -------------------------------------------------------------- |
| `fileName`  | `string`                                    | 是   | -      | Redux：`selectFileName`                                     | 当前文件显示名称；未打开时为空，Untitled 文件显示 `Untitled-n` |
